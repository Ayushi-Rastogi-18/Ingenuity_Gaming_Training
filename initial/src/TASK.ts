...
export class Game {
    ...
    constructor(app: Application) {
        ...
        preloader(list, () => {
            ...
            this.starting.on('click', () => {
                ...
                this.audio.src = '../assets/audio/spinning.mp3';
    ...
    createStartingScreen() {
        ...
        loading.scale.set(2.6);
        this.starting.addChild(loading);
        const welcome = this.createText("START GAME", this.app.view.width / 2, 30.66, 0.5);
    ...
    createEndingScreen() {
        ...
        loading.scale.set(2.3);
        this.ending.addChild(loading);
        const welcome = this.createText("PLAY AGAIN", this.app.view.width / 2, 30.66, 0.5);
    ...
    createcards() {
        shuffleCards(allcards).forEach((data) => {
            ...
            card.on('click', () => {
                ...
                this.audio.src = '../assets/audio/shuff.wav';
                this.audio.play();
                if (card.back) {
    ...
    alignCards() {
        const w = C_width;
        const h = C_height;
        const borderSide = C_width/2;
        const borderTop = C_height/2;
        const gap = 10;
        let index = 0;
        for (let i = 0; i < 6; i++) {
            for (let j = 0; j < 8; j++) {
    ...
    result() {
        ...
        if (this.card1 && this.card2) {
            if (this.card1.name == this.card2.name) {
                ...
                this.audio.src = '../assets/audio/open.wav';
                this.audio.play();
                ...
                this.audio.src = '../assets/audio/spinning.wav';
                this.audio.play();
                gsap.fromTo([this.card1, this.card2], { rotation: 0 }, {
    ...
    gameOver() {
        if (this.playing == true && this.game.children.length == 0) {
            ...
            audio.src = '../assets/audio/wingame.wav';
            console.log(audio);
