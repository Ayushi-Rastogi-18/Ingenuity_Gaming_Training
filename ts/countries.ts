...
export let country_list: Array<Object> = [
    { name: 'USA', code: 'US' },
    { name: 'Spain', code: 'ES' },
    { name: 'Italy', code: 'IT' },
    { name: 'France', code: 'FR' },
...
const search_country_element: HTMLSelectElement = document.querySelector(".search-country");
const country_list_element: HTMLSelectElement = document.querySelector(".country-list");
const chang_country_btn: HTMLButtonElement = document.querySelector(".change-country");
const close_list_btn: HTMLButtonElement = document.querySelector(".close");
const input: HTMLInputElement = document.getElementById('search-input') as HTMLInputElement;

// CREATE TE COUNTRY LIST
function createCountryList(){
    const num_countries: number = country_list.length;
    let i: number = 0, ul_list_id: string;

    country_list.forEach( (country, index:number) => {
...
setTimeout(()=>Array.from(document.getElementsByClassName("xyz")).forEach(ele=>{
    ele['onclick']=()=>{
        abc(ele.id);
    }
...
function resetCountryList(): void{
    country_list.forEach( country => {
        document.getElementById(country['name']).classList.remove("hide");
    })
}